...
- [十 透明代理](#十-透明代理)
  - [1. OpenAI 兼容对话代理](#proxy-chat)
- [十一 健康检查](#十一-健康检查)
  - [1. 根健康检查](#health)
  - [2. API 健康检查](#health-api)
- [十二 可观测](#十二-可观测)
  - [1. Worker 可观测统计](#monitor-stats)
...
智能体每轮对话后调用，将对话数据或任务过程写入记忆系统。当前实现为**只写入原始交互记录（L0）**，记忆的抽取（L1）、场景聚合（L2）、画像生成（L3）由后台 worker 异步完成，本接口即时返回落库确认。
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `accepted` | bool | 固定 `true`，表示 L0 记录已落库 |
| `session_id` | string | 生效的会话标识 |
| `l0_count` | int | 本次写入的 L0 记录条数 |
| `record_ids` | string[] | 本次写入的 L0 记录 ID 列表（`rec_xxx`） |
...
```json
{
  ...
  "data": {
    ...
    "l0_count": 3,
    "record_ids": ["rec_abc", "rec_def", "rec_ghi"]
  }
}
```
...
- 只写 L0，不在本接口内执行记忆抽取；L1 抽取由后台 `l1_worker` 按游标异步完成
- 本接口延迟为毫秒级（仅落库），不包含 LLM 调用
...
session 类型落库：`messages` 逐条写为 L0 记录，`session_summary` 额外写为一条 `role=session_summary` 的记录，`session_time`/`session_source` 存入 `extra_meta`，供后台 L1 抽取使用。
```
...
```json
{
  ...
  "data": {
    ...
    "session_id": "sess_abc123",
    "l0_count": 4,
    "record_ids": ["rec_001", "rec_002", "rec_003", "rec_summary"]
  }
}
```
...
```
POST /api/v1/memory/list?user_id=xxx&scene_id=xxx&task_id=xxx&session_id=xxx&page=1&page_size=20
```
...
多信号融合检索是记忆系统的核心调用接口。一轮调用内部执行：语义向量检索（Qdrant）→ T_MEMORY_VECTOR 桥接 → T_MEMORY 取权威数据 → 四维加权重排（向量相关度 + 时间新近性 + 重要性 + 置信度）。
...
- `relevance_score` 计算公式：`vector_score×0.6 + recency×0.15 + importance×0.15 + confidence×0.1`
...
返回该用户在该场景下的 L3 画像（persona 自由文本），消费 L2 场景块（硬隔离）。scene_id 由当前 Agent 绑定场景推导，不跨场景聚合。
...
|------|------|:---:|--------|------|
| `user_id` | string | ✅ | — | 用户标识 |
| `max_memories` | int | ❌ | 50 | 保留字段，当前实现未使用 |

**响应**

| 参数 | 类型 | 说明 |
|------|------|------|
| `persona` | string | L3 画像自由文本 |
| `scene_id` | string | 所属场景标识 |
| `changed_scenes` | int | 本次参与画像生成的变化场景数 |
...
```json
{
  "code": 0,
  "data": {
    "persona": "该用户是一名Go后端开发工程师，已有8年经验。偏好简洁沟通，技术选型保守倾向成熟方案。当前在开发OA系统，关注交付时效。",
    "scene_id": "customer_service",
    "changed_scenes": 3
  }
}
...
- scene_id 由当前 Agent 的绑定场景推导（硬隔离），Agent 未绑定场景时返回错误 `SCENE_REQUIRED`
- 无变化场景时返回已有画像（`changed_scenes` 为 0）
- 画像为自由文本，非结构化字段
...
透明代理（Proxy）接收外部 Agent 的 LLM 请求，完成「记忆召回注入 → 上游 LLM 转发 → 异步写回」，实现 OpenAI 兼容的无感记忆接入。
...
| 参数 | 类型 | 位置 | 必填 | 说明 |
|------|------|------|:---:|------|
| `space_id` | string | Path | ✅ | 空间标识，映射到 `{user_id, api_key, scene_id, agent_id}`（`spaces.yaml` 配置） |
| `messages` | array | Body | ✅ | OpenAI 格式对话消息 |
| `model` | string | Body | ❌ | 透传的模型名，默认 `deepseek-chat` |

会话映射：sessionKey 从 Header 提取（候选 `x-conversation-id` / `x-session-id` / `x-claude-code-session-id` / `x-deepseek-harness-session-id` / `x-chat-id` / `x-thread-id`，fallback 为 spaceId），映射到 session_id（落库，重启不丢）。

**响应**

OpenAI 兼容 chat.completion 结构：
...
- 未知 spaceId → 404
- 上游 LLM 转发失败 → 502
- 召回注入：调 `/memory/context`（偏好 + 事实，top_k=5）注入 user prompt
- 写回：异步后台任务写本轮对话，失败不阻塞响应
...
|------|------|------|
| `status` | string | 固定 `"ok"` |
| `database` | bool | 数据库连接是否正常 |

---

## 十二 可观测

<a id="monitor-stats"></a>

### 1. Worker 可观测统计

返回 L1/L2/L3 三级 worker 的累计处理统计、失败率、积压数、产出速率。
...
```json
{
  ...
  "data": {
    "uptime_seconds": 3600,
    "layers": {
      "l1": {"success": 120, "failed": 3, "failure_rate": 0.0244, "backlog": 5, "rate_per_min": 12},
      "l2": {"success": 40, "failed": 1, "failure_rate": 0.0244, "backlog": 2, "rate_per_min": 4},
      "l3": {"success": 8, "failed": 0, "failure_rate": 0.0, "backlog": 0, "rate_per_min": 1}
    }
  }
}
```
...
| `layers.lX.success` / `failed` | 该层累计成功/失败次数（进程内，重启归零） |
| `layers.lX.failure_rate` | 失败率 |
| `layers.lX.backlog` | 积压数（从 DB 实时查） |
| `layers.lX.rate_per_min` | 近 60 秒产出速率 |
