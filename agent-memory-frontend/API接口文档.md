...
与同步写入完全一致（`AsyncWriteRequest` 与 `MemoryWriteRequest` 共用同一字段结构），支持 `dialogue` / `session` / `task_process` / `metadata`。
...
| `status` | string | ❌ | 新状态：`active` / `deleted` / `expired` / `conflict` |
...
| `conflict_count` | int | 标记冲突的数量 |
| `details[]` | array | 每条记忆的处理详情 |
| `details[].action` | string | 处理动作：`keep_new` / `merge` / `discard` / `update_existing` / `conflict` |
